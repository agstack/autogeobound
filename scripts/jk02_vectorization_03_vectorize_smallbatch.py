...
n_minibatches = int(len(meta_df_batch) // minibatch_size + 1)

for j in range(n_minibatches):
    print(f"start minibatch {j}")
    start_idx = j * minibatch_size
    end_idx = min(len(meta_df), (j+1) * minibatch_size - 1)

    meta_df_minibatch = meta_df_batch.iloc[start_idx:end_idx]

    gdf_batch = gpd.GeoDataFrame()

    for i, row in tqdm(meta_df_minibatch.iterrows()):
        tile_id = row["tile_id"]
        batch = row["batch"]
    ...
    res_file = os.path.join(data_dir, 'res/india_{:02d}-{:02d}.gpkg'.format(batch, j))
